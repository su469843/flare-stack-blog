...
export function FriendLinkCard({ link }: FriendLinkCardProps) {
  ...
  return (
    <a
      ...
      className="ds-card-flat group flex items-start gap-4 p-4"
    >
      {/* Logo */}
      <div className="shrink-0 w-10 h-10 rounded-lg bg-muted/30 border border-border/40 flex items-center justify-center overflow-hidden transition-transform duration-300 group-hover:scale-105">
        {link.logoUrl ? (
          <img
            src={link.logoUrl}
            alt={link.siteName}
